...
# Reproduce the bug
Fuzz the message send by node 0 and drop the Write message from node 2 and node 3 to node 1 in epoch 0. For the counter client, set "increment" to 1, "number of operations" to 10. We can find that The cluster is unable to process 10 increment requests correctly and cannot select leaders normally after request processing fails.
